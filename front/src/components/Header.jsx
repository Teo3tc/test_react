...
import {useStore} from '../store/store'
 ...
 export const Header = ()=>{
    const itemInPanier = useStore(state => state.itemInPanier)
    const showPanier = useStore(state => state.showPanier)

    return (
      <header className="cl1 rw1 grid-full-fluid w100 t-rltv">
        <div className="cl1 rw1 grid-full-fluid w90 xctr">
          <div id="menu" 
            className="cl1 rw1 grid-full-fluid" 
            onClick={()=> showPanier(false)}
          >
            <span className="t-white yctr">HOME</span>
          </div>
          <div id='icone__panner ' 
            className="xend cl2 rw1 grid-full-fluid" 
            onClick={()=> showPanier(true)}
          >
            <svg className="cl1 rw1 svg__pannier"  viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {itemInPanier !== 0 && 
            <div id='circle' className="t-abslt">
              <span>{itemInPanier}</span>
            </div>
            }
          </div>
        </div>
